...
/**
 * BepHax's TrailFollower, ported as-is.
 *
 * <p>Copied from the NEW-SRC drop of BepHax and translated mechanically —
 * package, Yarn mappings, our category and util paths — with the logic left
 * exactly as written there: forward-weighted average over the recent trail
 * chunks, a committed heading with a turn-rate cap and a reverse lock, and a
 * sweep search when chunks stop arriving. The module name and the setting
 * names are theirs, which also happens to keep any saved values applying.
 */
public class TrailFollower extends Module {
    ...
    public final Setting<Integer> directionWeightingMultiplier = this.sgAdvanced.add(new IntSetting.Builder()
        ...
        .min(2)
        .sliderMax(10)
        .visible(() -> this.directionWeighting.get() != DirectionWeighting.NONE)
        .build()
    );
    ...
    public final Setting<SearchBehavior> searchBehavior = this.sgAdvanced.add(new EnumSetting.Builder<SearchBehavior>()
        ...
        .defaultValue(SearchBehavior.SWEEP)
        .build()
    );
    ...
    public final Setting<Boolean> reverseLock = this.sgAdvanced.add(new BoolSetting.Builder()
        .name("reverse-lock")
        .description("Ignores chunks too far off the committed trail direction, so chunks loading late behind you (server lag) can't turn you back the way you came.")
        .defaultValue(true)
        .build()
    );
