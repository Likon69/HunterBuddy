...
/** Reference parity: mlep.modules.RocketFly. 100% faithful port. */
public class RocketFly extends Module {
    ...
    private final Setting<Integer> fireworkDelay = sgGeneral.add(
        new IntSetting.Builder().name("timed-delay")
            .description("The delay between firework usages in milliseconds.")
            .defaultValue(4000).sliderRange(0, 10000).build()
    );
    private final Setting<Boolean> useManualY = sgGeneral.add(
        new BoolSetting.Builder().name("use-manual-y-level")
            .description("Use a manually set Y level instead of the Y level when activated.")
            .defaultValue(false).build()
    );
    private final Setting<Integer> manualYLevel = sgGeneral.add(
        new IntSetting.Builder().name("manual-y-level")
            .description("The Y level to maintain when using manual Y level.")
            .defaultValue(256).sliderRange(-64, 320)
            .visible(this.useManualY::get)
            .onChanged(val -> this.yTarget = val.intValue()).build()
    );

    public RocketFly() {
        super(HunterBuddyAddon.HUNTER_BUDDY_CATEGORY, "RocketFly",
            "Maintains a level Y-flight with fireworks and smooth pitch control.");
    }
    ...
    public void tickFlyLogic() {
        if (this.mc.player != null) {
            ...
            if (this.mc.player.isGliding()) {
                ...
                if (Math.abs(yDiff) > 10.0) this.targetPitch = (float) (Math.atan2(yDiff, 100.0) * (180.0 / Math.PI));
                else if (yDiff > 2.0) this.targetPitch = 10.0F;
                else if (yDiff < -2.0) this.targetPitch = -10.0F;
                else this.targetPitch = 0.0F;
                ...
                if (System.currentTimeMillis() - this.lastRocketUse > this.fireworkDelay.get()) {
                    this.tryUseFirework();
                }
            } else {
                if (!this.launched) {
    ...
    private int findEmptyHotbarSlot() {
        for (int i = 0; i < 9; i++) {
            if (this.mc.player.getInventory().getStack(i).isEmpty()) return i;
        }
        return -1;
    }
}