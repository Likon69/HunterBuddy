...
/** Reference parity: mlep.modules.Pitch40. 100% faithful port. */
public class Pitch40 extends Module {
    private final SettingGroup sgGeneral = settings.getDefaultGroup();
    private final SettingGroup sgFirework = settings.createGroup("Auto Firework");
    private final SettingGroup sgRecast = settings.createGroup("Auto Recast");
    public final Setting<Double> pitch40LowerBounds = sgGeneral.add(
        new DoubleSetting.Builder().name("pitch40-lower-bounds")
            .description("The bottom height boundary for pitch40. After descending below this boundary you will start pitching upwards. Synced with ElytraFly.")
            .defaultValue(360.0).min(-128.0).sliderRange(-64.0, 500.0)
            .onChanged(this::syncLowerBoundsToElytraFly).build()
    );
    public final Setting<Double> pitch40UpperBounds = sgGeneral.add(
        new DoubleSetting.Builder().name("pitch40-upper-bounds")
            .description("The upper height boundary for pitch40. When ascending above this boundary you will start pitching downwards. Synced with ElytraFly.")
            .defaultValue(420.0).min(-128.0).sliderRange(-64.0, 500.0)
            .onChanged(this::syncUpperBoundsToElytraFly).build()
    );
    public final Setting<Boolean> autoFirework = sgFirework.add(
        new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
            .name("auto-firework")
            .description("Uses a firework automatically when speed is too low or height drops below bounds.")
            .defaultValue(true).build()
    );
    public final Setting<Double> minSpeed = sgFirework.add(
        new DoubleSetting.Builder().name("min-speed")
            .description("Fire rocket when speed drops below this (blocks/sec). Normal Pitch40 maintains ~30-40 b/s.")
            .defaultValue(5.0).sliderRange(10.0, 50.0)
            .visible(this.autoFirework::get).build()
    );
    public final Setting<Integer> fireworkCooldownTicks = sgFirework.add(
        new meteordevelopment.meteorclient.settings.IntSetting.Builder()
            .name("cooldown-ticks")
            .description("Minimum ticks between firework uses.")
            .defaultValue(100).sliderRange(10, 100)
            .visible(this.autoFirework::get).build()
    );
    public final Setting<Boolean> autoRecast = sgRecast.add(
        new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
            .name("auto-recast")
    ...
    public Pitch40() {
        super(HunterBuddyAddon.HUNTER_BUDDY_CATEGORY, "Pitch40",
            "Utility for Pitch40 elytra flying. Syncs bounds with ElytraFly and auto-enables on reconnect.");
    }

    private Module getElytraFly() {
        if (this.elytraFly == null) this.elytraFly = Modules.get().get(ElytraFly.class);
    ...
    public void onDeactivate() {
        if (this.getElytraFly().isActive()) this.getElytraFly().toggle();
        ...
        if (this.mc.player != null && this.mc.world != null) {
            ...
            if (this.lastDimension != null && !this.lastDimension.equals(currentDimension)) {
                ...
                if (!elytraFlyActive) {
                    ...
                    if (wasGoingUp && !this.goingUp) this.peakY = playerY;
                    else if (!wasGoingUp && this.goingUp) this.troughY = playerY;
                    if (this.goingUp && playerY > this.peakY) this.peakY = playerY;
                    else if (!this.goingUp && playerY < this.troughY) this.troughY = playerY;
                ...
                if (this.elytraRecast == null || !this.elytraRecast.isActive() || !this.elytraRecast.isRecovering()) {
                    ...
                    if (needsBoost || emergency) {
                        ...
                        if (launchStatus >= 0) {
                            ...
                            if (launchStatus != 200) this.elytraSwapSlot = launchStatus;
                            this.peakY = playerY;
