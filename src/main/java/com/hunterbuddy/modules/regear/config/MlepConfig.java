...
/**
 * Reference parity: mlep.config.MlepConfig. Holds global Mlep-style
 * configuration (turn speed, illegal disconnect, telemetry toggles, ...).
 */
public class MlepConfig {
    public static Setting<MlepConfig.TurnSpeedMode> rotationTurnSpeedMode = new Builder<MlepConfig.TurnSpeedMode>()
        .defaultValue(MlepConfig.TurnSpeedMode.Normal).build();
    public static Setting<Double> rotationTurnSpeedCustom = new meteordevelopment.meteorclient.settings.DoubleSetting.Builder()
        .defaultValue(45.0).build();
    public static Setting<Boolean> illegalDisconnectButtonSetting = new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
        .build();
    public static Setting<Boolean> disableMeteorClientTelemetry = new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
        .build();
    public static Setting<Boolean> disableMeteorCapes = new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
        .build();
    public static Setting<Boolean> ignoreOverlayMessages = new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
        .build();
    public static Setting<List<String>> overlayMessageFilter = new meteordevelopment.meteorclient.settings.StringListSetting.Builder()
        .build();
    public static Setting<Utils.IllegalDisconnectMethod> illegalDisconnectMethodSetting = new Builder<Utils.IllegalDisconnectMethod>()
        .defaultValue(Utils.IllegalDisconnectMethod.Chat).build();
    public static Setting<SettingColor> enemyColorSetting = new meteordevelopment.meteorclient.settings.ColorSetting.Builder()
        .build();

    public static double getRotationTurnSpeed() {
        if (rotationTurnSpeedMode == null) return 45.0;
        TurnSpeedMode mode = (TurnSpeedMode) rotationTurnSpeedMode.get();
        return mode == TurnSpeedMode.Custom ? (Double) rotationTurnSpeedCustom.get() : mode.speed;
    }
    ...
    public static void initialize() {
        ...
        illegalDisconnectMethodSetting = sgMlep.add(
            new Builder<Utils.IllegalDisconnectMethod>().name("illegal-disconnect-method")
        ...
        rotationTurnSpeedCustom = sgMlep.add(
            new meteordevelopment.meteorclient.settings.DoubleSetting.Builder()
                ...
                .defaultValue(45.0).min(1.0).max(180.0).sliderRange(5.0, 180.0)
                .visible(() -> rotationTurnSpeedMode.get() == TurnSpeedMode.Custom).build()
        );
        ...
        overlayMessageFilter = sgMlep.add(
            new meteordevelopment.meteorclient.settings.StringListSetting.Builder()
                ...
                .description("Overlay messages will be ignored if they match any of the provided filters.")
                .defaultValue(List.of("2b2t.org")).visible(ignoreOverlayMessages::get).build()
        );
    }
    ...
    public enum TurnSpeedMode {
        ...
        TurnSpeedMode(double speed) { this.speed = speed; }
    }
}