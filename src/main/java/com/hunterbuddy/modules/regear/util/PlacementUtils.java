...
public class PlacementUtils {
   ...
   public static boolean placeHit(BlockHitResult hit, Hand hand, boolean swing) {
      if (MeteorClient.mc.player == null || MeteorClient.mc.interactionManager == null) {
         return false;
      }
      ...
      if (hit.isInsideBlock()) {
         ...
         MeteorClient.mc.interactionManager.interactBlock(MeteorClient.mc.player, hand, hit);
      }
   ...
   public static boolean requestRotationForHit(BlockHitResult hit, boolean syncMovement) {
      ...
      if (syncMovement) {
         rotationUtils.setRotationFull(rotations[0], rotations[1]);
      } else {
         rotationUtils.setRotationSilent(rotations[0], rotations[1]);
      }
   ...
   public static boolean placeAt(BlockPos pos, FindItemResult block, boolean rotate, boolean swing, boolean strictDirection, double reach) {
      ...
      if (rotate) {
         requestRotationForHit(hitResult, false);
      }
   ...
   public static boolean canPlace(BlockPos pos, boolean strictDirection) {
      ...
      if (!MeteorClient.mc.world.getBlockState(pos).isReplaceable()) {
         return false;
      }

      if (!MeteorClient.mc.world.canPlace(Blocks.OBSIDIAN.getDefaultState(), pos, ShapeContext.absent())) {
         return false;
      }
   ...
   public static Direction getPlaceSide(BlockPos pos) {
      ...
      if (!MeteorClient.mc.world.getBlockState(pos.down()).isReplaceable()) {
         return Direction.DOWN;
      }

      for (Direction side : Type.HORIZONTAL) {
         BlockPos neighbor = pos.offset(side);
         if (!MeteorClient.mc.world.getBlockState(neighbor).isReplaceable()) {
            return side;
         }
      }
      ...
      if (MeteorClient.mc.player != null && MeteorClient.mc.world != null) {
         ...
         for (int x = (int)Math.floor(playerBox.minX); x <= Math.floor(playerBox.maxX); x++) {
            for (int z = (int)Math.floor(playerBox.minZ); z <= Math.floor(playerBox.maxZ); z++) {
               ...
               if (!MeteorClient.mc.world.getBlockState(feetPos).getCollisionShape(MeteorClient.mc.world, feetPos).isEmpty()) {
                  feetBlocked = true;
               }

               BlockPos headPos = new BlockPos(x, (int)Math.floor(playerBox.maxY), z);
               if (!MeteorClient.mc.world.getBlockState(headPos).getCollisionShape(MeteorClient.mc.world, headPos).isEmpty()) {
                  headBlocked = true;
               }
