...
public class RotationUtils {
   ...
   public void onTickPost(Post event) {
      if (MeteorClient.mc.player != null && MeteorClient.mc.world != null) {
         if (this.isRotating()) {
            ...
            if (this.decaying) {
               if (this.decayTicks-- <= 0 || this.isAligned()) {
                  this.clearRotations();
               }
   ...
   public static float[] getRotationsTo(Entity entity, RotationUtils.HitVector hitVector) {
      if (MeteorClient.mc.player == null) {
   ...
   public static boolean isInFov(Vec3d from, Vec3d to, float fov) {
      if (fov >= 180.0F || MeteorClient.mc.player == null) {
         return fov >= 180.0F;
      }
   ...
   @Deprecated
   public void requestRotation(double yaw, double pitch, RotationUtils.RotationConfig config) {
   ...
   public static class RotationConfig {
      private RotationUtils.RotationMode mode = RotationUtils.RotationMode.SILENT;
      private double turnSpeed = 180.0;
      private int keepTicks = 1;
      private int decayTicks = 0;
      private int priority = 0;

      public RotationUtils.RotationConfig mode(RotationUtils.RotationMode mode) {
         this.mode = mode;
         return this;
      }

      public RotationUtils.RotationConfig turnSpeed(double v) {
         this.turnSpeed = v;
         return this;
      }

      public RotationUtils.RotationConfig keepTicks(int v) {
         this.keepTicks = v;
         return this;
      }

      public RotationUtils.RotationConfig decayTicks(int v) {
         this.decayTicks = v;
         return this;
      }

      public RotationUtils.RotationConfig priority(int v) {
         this.priority = v;
         return this;
      }

      public RotationUtils.RotationMode getMode() {
         return this.mode;
      }
