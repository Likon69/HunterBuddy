...
import com.hunterbuddy.modules.regear.util.LogUtil;
...
/**
 * Reference parity: mlep.modules.arealoader.AreaLoaderMode. 100% faithful port.
 * <p>Note on inter-module references: AreaLoaderMode uses several other mlep modules
 * (Pitch40, RocketFly, WaypointFollower) which we will port in subsequent steps.
 * Until those modules are ported, the runtime path that exercises those branches
 * is a no-op (the affected calls early-return on null references).
 */
public abstract class AreaLoaderMode {
    ...
    protected CurrentDimension currentDimension = CurrentDimension.UNKNOWN;
    ...
    protected boolean isSurvival() {
        return this.mc.player != null && this.mc.interactionManager != null
            ? this.mc.interactionManager.getCurrentGameMode().isSurvivalLike() : false;
    }

    protected CurrentDimension getDimension() {
        if (this.mc.world == null) return CurrentDimension.UNKNOWN;
        else if (this.isNether()) return CurrentDimension.NETHER;
        else return this.isEnd() ? CurrentDimension.END : CurrentDimension.OVERWORLD;
    }

    protected void checkDimensionChange() {
        if (this.isWorldReady()) {
            CurrentDimension newDimension = this.getDimension();
            if (this.flightModesInitialized && newDimension != this.currentDimension) {
                ...
                this.isInNether = newDimension == CurrentDimension.NETHER;
    ...
    protected void initializeFlightModes() {
        if (!this.flightModesInitialized) {
            if (!this.isWorldReady()) {
                ...
                this.isInNether = this.currentDimension == CurrentDimension.NETHER;
                if (!this.isSurvival()) {
                    ...
                    this.debugInfo("AreaLoader: Initializing flight modes. currentDimension=%s, dimension=%s",
                    ...
                    if (this.currentDimension == CurrentDimension.NETHER) {
                        ...
                        this.currentDimension = CurrentDimension.NETHER;
                        this.isInNether = true;
                    } else {
                        if (this.currentDimension == CurrentDimension.END) {
                        ...
                        if (this.enabledOverworldMode == AreaLoader.OverworldFlightMode.PITCH40) {
                            meteordevelopment.meteorclient.systems.modules.Module pitch40Module = Modules.get().get(Pitch40.class);
                            if (pitch40Module != null) {
                                meteordevelopment.meteorclient.settings.Setting<Boolean> autoRecastSetting = (meteordevelopment.meteorclient.settings.Setting<Boolean>) pitch40Module.settings.get("auto-recast");
                                ...
                                if ((Boolean) this.searchArea.pitch40AutoFirework.get()) {
                                    meteordevelopment.meteorclient.settings.Setting<Boolean> autoFireworkSetting = (meteordevelopment.meteorclient.settings.Setting<Boolean>) pitch40Module.settings.get("auto-firework");
                                ...
                                meteordevelopment.meteorclient.settings.Setting<Boolean> autoBoundAdjustSetting = (meteordevelopment.meteorclient.settings.Setting<Boolean>) pitch40Module.settings.get("auto-bound-adjust");
    ...
    private void cleanupOverworldModules() {
        ...
        if (pitch40Module != null) {
            if (this.pitch40SettingsModified) {
                meteordevelopment.meteorclient.settings.Setting<Boolean> autoBoundAdjustSetting = (meteordevelopment.meteorclient.settings.Setting<Boolean>) pitch40Module.settings.get("auto-bound-adjust");
                if (autoBoundAdjustSetting != null) autoBoundAdjustSetting.set(this.oldAutoBoundAdjustValue);
                this.pitch40SettingsModified = false;
            }
            if (this.pitch40AutoRecastModified) {
                meteordevelopment.meteorclient.settings.Setting<Boolean> autoRecastSetting = (meteordevelopment.meteorclient.settings.Setting<Boolean>) pitch40Module.settings.get("auto-recast");
                if (autoRecastSetting != null) autoRecastSetting.set(this.oldAutoRecastValue);
                this.pitch40AutoRecastModified = false;
            }
            if (this.pitch40AutoFireworkModified) {
                meteordevelopment.meteorclient.settings.Setting<Boolean> autoFireworkSetting = (meteordevelopment.meteorclient.settings.Setting<Boolean>) pitch40Module.settings.get("auto-firework");
                if (autoFireworkSetting != null) autoFireworkSetting.set(this.oldAutoFireworkValue);
    ...
    protected boolean isBaritoneIdle() {
        if (!BaritoneHelper.hasElytraProcess()) return true;
        try {
    ...
    protected boolean isStuck() {
        if (this.mc.player != null && this.currentBaritoneGoal != null) {
            ...
            if (now - this.lastMovementCheckTime < 1000L) return false;
    ...
    protected boolean needsNewGoal() {
        if (this.currentBaritoneGoal == null) return true;
        else if (this.hasReachedBaritoneGoal()) return true;
        else {
            long now = System.currentTimeMillis();
            if (now - this.lastGoalSetTime < 2000L) return false;
    ...
    protected String getDimensionSuffix() {
        ...
        try {
            String suffix;
            if (this.mc.world.getRegistryKey().equals(World.NETHER)) suffix = "_nether";
            else if (this.mc.world.getRegistryKey().equals(World.END)) suffix = "_end";
            else suffix = "_overworld";
    ...
    protected File getJsonFile(String fileName) {
        try {
            ...
            if (saveName == null || saveName.trim().isEmpty()) saveName = "default";
    ...
    public void clear(String mode) {
        ...
        if (file != null && file.exists()) {
            if (file.delete()) this.debugInfo("✓ Cleared saved " + mode + " progress from: " + file.getAbsolutePath());
            else this.debugInfo("✗ Failed to delete " + mode + " save file: " + file.getAbsolutePath());
        }
    }
    ...
    public void clearAll() {
        ...
        this.resetState();
        if (deletedCount == 0) this.debugInfo("No save files found to clear, but in-memory state has been reset.");
        else this.debugInfo("Successfully deleted " + deletedCount + " save file(s) and reset state. Ready to start fresh!");
    }
