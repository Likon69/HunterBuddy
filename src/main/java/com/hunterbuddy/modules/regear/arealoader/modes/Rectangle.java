...
/** Reference parity: mlep.modules.arealoader.modes.Rectangle. 100% faithful port. */
public class Rectangle extends AreaLoaderMode {
    ...
    public void onActivate() {
        ...
        if (file == null) {
            ...
            if (startPos.equals(new BlockPos(0, 0, 0))) startPos = playerPos;
            if (targetPos.equals(new BlockPos(0, 0, 0))) targetPos = startPos.add(10000, 0, 10000);
            float initialYaw = startPos.getX() < targetPos.getX() ? -90.0F : 90.0F;
            this.pd = new PathingDataRectangle(startPos, targetPos, playerPos, initialYaw, true, playerPos.getZ());
            ...
            this.pd = new PathingDataRectangle(startPos, targetPos, playerPos, initialYaw, true, playerPos.getZ());
            this.goingToStart = false;
            this.debugInfo("Rectangle started from player position " + playerPos.toShortString()
                + ". Will scan from " + startPos.toShortString() + " to " + targetPos.toShortString());
        } else {
            try {
                this.debugInfo("Loading save file: " + file.getAbsolutePath() + " (" + file.length() + " bytes)");
                FileReader reader = new FileReader(file);
                this.pd = (PathingDataRectangle) GSON.fromJson(reader, PathingDataRectangle.class);
                reader.close();
                if (!this.isRectanglePathingDataValid(this.pd)) {
                    ChatUtils.error("Rectangle save file is missing required fields. Disabling to prevent data loss.");
                    this.pd = null;
                    this.disable();
                    return;
                }
    ...
    private boolean isRectanglePathingDataValid(PathingDataRectangle data) {
    ...
    private void printRectangleEstimate() {
        ...
        this.debugInfo("Completion will take an estimated %02d hours %02d minutes %02d seconds with boatfly at a speed of %.2f and a gap of %d chunks between paths.",
            hours, minutes, seconds, speedBPS, this.searchArea.rowGap.get());
    }
    ...
    private void onTickOverworld() {
        if (this.goingToStart) {
            ...
            if (Math.sqrt(this.mc.player.getBlockPos()
                .getSquaredDistance(this.pd.currPos.getX(), this.mc.player.getY(), this.pd.currPos.getZ())) < 5.0) {
                this.goingToStart = false;
            ...
            if (Math.sqrt(this.mc.player.getBlockPos()
                .getSquaredDistance(this.pd.targetPos.getX(), this.mc.player.getY(), this.pd.targetPos.getZ())) < 20.0) {
                this.onComplete();
            } else if (this.pd.mainPath && (
                this.pd.yawDirection == -90.0F && this.mc.player.getX() >= Math.max(this.pd.initialPos.getX(), this.pd.targetPos.getX())
                    || this.pd.yawDirection == 90.0F && this.mc.player.getX() <= Math.min(this.pd.initialPos.getX(), this.pd.targetPos.getX()))) {
    ...
    private void onTickNether() {
        if (this.searchArea.netherPathMode.get() != AreaLoader.NetherPathMode.BARITONE_ELYTRA) {
            ...
            if (this.recovering && this.recoveryTarget != null) {
                double distToRecovery = Math.sqrt(this.mc.player.getBlockPos()
                ...
                if (elytraRecovering) {
                    ...
                    if (distToRecovery < reachDist) {
                        this.debugInfo("Rectangle: Recovery complete. Resuming from X=%d Z=%d", this.recoveryTarget.getX(), this.recoveryTarget.getZ());
                        this.recovering = false;
                        this.recoveryTarget = null;
                        this.offTrackCounter = 0;
                ...
                double distToSaved = Math.sqrt(this.mc.player.getBlockPos()
                ...
                if (distToSaved < reachDist) {
                    ...
                    this.nextWaypointTarget = null;
                    this.debugInfo("Rectangle: Reached saved position. Resuming at X=%d Z=%d, yaw=%.1f, mainPath=%b",
                        this.pd.currPos.getX(), this.pd.currPos.getZ(), this.pd.yawDirection, this.pd.mainPath);
                } else if (this.needsNewGoal()) {
                    this.setBaritoneGoal(this.pd.currPos);
                }
            } else {
                double distToTarget = Math.sqrt(this.mc.player.getBlockPos()
                ...
                if (distToTarget < reachDist) {
                    ...
                    if (this.nextWaypointTarget != null) {
                        this.debugInfo("Rectangle: New waypoint at X=%d Z=%d (mainPath=%b, yaw=%.1f)",
                            this.nextWaypointTarget.getX(), this.nextWaypointTarget.getZ(), this.pd.mainPath, this.pd.yawDirection);
                    ...
                    if (!elytraRecovering) {
                        ...
                        if (this.recoveryCompletedTime > 0L && timeSinceRecovery < 5000L) {
                            ...
                            if (offTrackDist > 200.0) {
                                ...
                                if (this.offTrackCounter >= 10) {
                                    ...
                                    if (this.lastKnownGoodPosition != null && distToLastGood > reachDist * 2) {
                                        ...
                                        ChatUtils.info("Rectangle: Saved recovery point at X=%d Z=%d. Navigating back.",
                                            this.lastKnownGoodPosition.getX(), this.lastKnownGoodPosition.getZ());
                                    } else {
                                        this.recoveryTarget = this.pd.currPos;
                                        super.saveToJson(false, this.pd);
                                        ChatUtils.info("Rectangle: Recovering to last saved position X=%d Z=%d",
                            ...
                            double distToWaypoint = Math.sqrt(this.mc.player.getBlockPos()
                            ...
                            if (distToWaypoint < reachDist) {
                                this.debugInfo("Rectangle: Reached waypoint at X=%d Z=%d", this.nextWaypointTarget.getX(), this.nextWaypointTarget.getZ());
                                if (this.pd.mainPath) {
                                ...
                                this.calculateNextWaypoint();
                                if (this.nextWaypointTarget != null) {
                                    this.debugInfo("Rectangle: Next waypoint at X=%d Z=%d", this.nextWaypointTarget.getX(), this.nextWaypointTarget.getZ());
                                    this.setBaritoneGoal(this.nextWaypointTarget);
                                }
                            } else if (this.needsNewGoal()) {
    ...
    private void calculateNextWaypoint() {
        ...
        int targetZ;
        if (this.pd.mainPath) {
            if (this.pd.yawDirection == -90.0F) targetX = Math.max(this.pd.initialPos.getX(), this.pd.targetPos.getX());
            else targetX = Math.min(this.pd.initialPos.getX(), this.pd.targetPos.getX());
            ...
            if (this.pd.yawDirection == 0.0F) targetZ = this.pd.lastCompleteRowZ + blockGap;
            else targetZ = this.pd.lastCompleteRowZ - blockGap;
        }
    ...
    private double getRectangleOffTrackDistance() {
        if (this.pd == null) return 0.0;
        BlockPos currentPos = this.mc.player.getBlockPos();
        if (this.pd.mainPath) return Math.abs(currentPos.getZ() - this.pd.lastCompleteRowZ);
        else return this.nextWaypointTarget != null ? Math.abs(currentPos.getX() - this.nextWaypointTarget.getX()) : 0.0;
    }
