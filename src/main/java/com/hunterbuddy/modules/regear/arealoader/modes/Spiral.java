...
public class Spiral extends AreaLoaderMode {
   ...
   public void onActivate() {
      ...
      if (file == null) {
         ...
         this.pd = new Spiral.PathingDataSpiral(this.mc.player.getBlockPos(), this.mc.player.getBlockPos(), -90.0F, true, 0, 0);
         ...
         try {
            ...
            this.pd = (Spiral.PathingDataSpiral)GSON.fromJson(reader, Spiral.PathingDataSpiral.class);
            reader.close();
            ...
            if (this.pd == null) {
               ...
               if (distFromOrigin > 1000.0) {
                  corrupted = true;
                  corruptionReason = String.format("spiralWidth/Height are 0 but currPos is %.0f blocks from origin - save may have been reset", distFromOrigin);
               }
            }

            if (corrupted) {
               ChatUtils.error("SAVE FILE APPEARS CORRUPTED: " + corruptionReason, new Object[0]);
               ChatUtils.error("NOT starting fresh to protect your progress. Please fix the save file manually.", new Object[0]);
               ChatUtils.error("Save file location: " + file.getAbsolutePath(), new Object[0]);
               ChatUtils.error("Disabling module to prevent data loss.", new Object[0]);
               this.pd = null;
               this.disable();
               return;
            }
            ...
            this.debugInfo(
               "Spiral state: width=%d, height=%d, yaw=%.1f, mainPath=%b", this.pd.spiralWidth, this.pd.spiralHeight, this.pd.yawDirection, this.pd.mainPath
            );
         } catch (Exception e) {
            ChatUtils.error("Failed to load saved path: " + e.getMessage(), new Object[0]);
            ChatUtils.error("NOT starting fresh to protect your progress. Please check the save file.", new Object[0]);
            ChatUtils.error("Save file location: " + file.getAbsolutePath(), new Object[0]);
            this.pd = null;
            this.disable();
            return;
   ...
   private void onTickOverworld() {
      if (!this.isInNether) {
         if (this.goingToStart) {
            ...
            if (offTrackDistance > ((Integer)this.searchArea.spiralMaxOffTrackDistance.get()).intValue()) {
               this.debugInfo(
               ...
               super.saveToJson(false, this.pd);
               ChatUtils.info(
                  "Saved recovery point at X=%d Z=%d. Navigating back to resume spiral.", new Object[]{expectedPos.getX(), expectedPos.getZ()}
               );
   ...
   private boolean hasReachedCorner(BlockPos corner) {
      double distToCorner = Math.sqrt(
         this.mc.player.getBlockPos().getSquaredDistance(corner.getX(), this.mc.player.getY(), corner.getZ())
      );
      if (distToCorner < ((Integer)this.searchArea.spiralCornerReachDistance.get()).intValue()) {
         return true;
            ...
            if (this.goingToStart) {
               ...
               double distToCorner = Math.sqrt(
                  this.mc
                     .player
                     .getBlockPos()
                     .getSquaredDistance(this.nextCornerTarget.getX(), this.mc.player.getY(), this.nextCornerTarget.getZ())
               );
               if (distToCorner < reachDist) {
                  BlockPos snappedCorner = this.nextCornerTarget;
                  this.debugInfo(
                     "Spiral: Reached corner at X=%d Z=%d, yaw was %.1f", snappedCorner.getX(), snappedCorner.getZ(), this.pd.yawDirection
                  );
                  this.pd.yawDirection = this.normalizeYaw(this.pd.yawDirection + 90.0F);
                  this.pd.initialPos = new BlockPos(snappedCorner.getX(), this.pd.initialPos.getY(), snappedCorner.getZ());
                  if (this.pd.mainPath) {
   ...
   private List<Spiral.SpiralCornerState> generateSpiralCorners(int originX, int originZ, int blockGap, int maxCorners) {
      List<Spiral.SpiralCornerState> corners = new ArrayList<>();
      int x = originX;
      int z = originZ;
      float yaw = 270.0F;
      boolean mainPath = true;
      int spiralWidth = 0;
      int spiralHeight = 0;
      corners.add(new Spiral.SpiralCornerState(x, z, spiralWidth, spiralHeight, yaw, mainPath, 0));

      for (int i = 1; i <= maxCorners; i++) {
         float normYaw = this.normalizeYaw(yaw);
         if (mainPath) {
            if (normYaw >= 225.0F && normYaw < 315.0F) {
               x += blockGap + spiralWidth;
            } else {
               x -= blockGap + spiralWidth;
            }
         } else if (!(normYaw >= 315.0F) && !(normYaw < 45.0F)) {
            z -= blockGap + spiralHeight;
         } else {
            z += blockGap + spiralHeight;
         }
         ...
         corners.add(new Spiral.SpiralCornerState(x, z, spiralWidth, spiralHeight, yaw, mainPath, i));
      }

      return corners;
   }

   private Spiral.SpiralCornerState findClosestCorner(int originX, int originZ, int targetX, int targetZ, int blockGap) {
      ...
      List<Spiral.SpiralCornerState> corners = this.generateSpiralCorners(originX, originZ, blockGap, estimatedCorners);
      Spiral.SpiralCornerState closest = null;
      double closestDist = Double.MAX_VALUE;

      for (Spiral.SpiralCornerState corner : corners) {
   ...
   public boolean validateManualCoordinates(int originX, int originZ, int targetX, int targetZ, int blockGap, boolean apply) {
      ...
      if (closestLeg == null) {
         ChatUtils.error("Could not calculate spiral path. Check your settings.", new Object[0]);
         return false;
      }
      ...
      if (apply) {
         if (distanceToLeg > blockGap * 2) {
            ChatUtils.error("Position is too far from spiral path (%.0f blocks). Max tolerance: %d blocks.", new Object[]{distanceToLeg, blockGap * 2});
            ChatUtils.error("Make sure you entered the correct origin coordinates where the spiral started.", new Object[0]);
            return false;
         }

         int resumeX = distanceToLeg < 50.0 ? targetX : projected[0];
         int resumeZ = distanceToLeg < 50.0 ? targetZ : projected[1];
         Spiral.PathingDataSpiral newPd = new Spiral.PathingDataSpiral(new BlockPos(closestLeg.startX, 64, closestLeg.startZ),
   ...
   public int[] snapToNearestCorner(int originX, int originZ, int targetX, int targetZ, int blockGap) {
      ...
      Spiral.SpiralLegInfo closestLeg = this.findClosestLeg(originX, originZ, targetX, targetZ, blockGap);
      if (closestLeg == null) {
         ChatUtils.error("Could not calculate spiral path.", new Object[0]);
         return null;
      } else {
         int[] projected = closestLeg.projectOntoLeg(targetX, targetZ);
   ...
   public boolean applyFromNextCorner(int originX, int originZ, int targetX, int targetZ, int blockGap) {
      ...
      Spiral.SpiralLegInfo closestLeg = this.findClosestLeg(originX, originZ, targetX, targetZ, blockGap);
      if (closestLeg == null) {
         ChatUtils.error("Could not find your position on the spiral.", new Object[0]);
         return false;
      }
      ...
      Spiral.PathingDataSpiral newPd = new Spiral.PathingDataSpiral(new BlockPos(cornerX, 64, cornerZ), new BlockPos(cornerX, 64, cornerZ), newYaw, newMainPath, newWidth, newHeight
      );
      newPd.spiralOrigin = new BlockPos(originX, 64, originZ);
      this.debugInfo("Getting save file for '%s'", this.toString());
      File file = this.getJsonFile(this.toString());
      if (file == null) {
         ChatUtils.error("Failed to get save file path. Check that you're in a world.", new Object[0]);
         return false;
      }
