...
/**
 * Reference parity: mlep.modules.arealoader.AreaLoader. 100% faithful port.
 */
public class AreaLoader extends Module {
    private final meteordevelopment.meteorclient.settings.SettingGroup sgGeneral = this.settings.getDefaultGroup();
    private final meteordevelopment.meteorclient.settings.SettingGroup sgFlight = this.settings.createGroup("Flight");
    private final meteordevelopment.meteorclient.settings.SettingGroup sgNether = this.settings.createGroup("Nether");
    private final meteordevelopment.meteorclient.settings.SettingGroup sgDebug = this.settings.createGroup("Debug");

    public final meteordevelopment.meteorclient.settings.Setting<AreaLoaderModes> chunkLoadMode = this.sgGeneral.add(
    ...
    public final meteordevelopment.meteorclient.settings.Setting<BlockPos> startPos = this.sgGeneral.add(
        new meteordevelopment.meteorclient.settings.BlockPosSetting.Builder()
            .name("start-position").description("The coordinates to start the rectangle at. Y Pos is ignored")
            .defaultValue(new BlockPos(0, 0, 0))
            .visible(() -> this.chunkLoadMode.get() == AreaLoaderModes.Rectangle).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<BlockPos> targetPos = this.sgGeneral.add(
        new meteordevelopment.meteorclient.settings.BlockPosSetting.Builder()
            .name("end-position").description("The coordinates to end the rectangle at. Y Pos is ignored")
            .defaultValue(new BlockPos(0, 0, 0))
            .visible(() -> this.chunkLoadMode.get() == AreaLoaderModes.Rectangle).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<Integer> rowGap = this.sgGeneral.add(
        new meteordevelopment.meteorclient.settings.IntSetting.Builder()
            .name("path-gap").description("The amount of chunks to space between each chunk path.")
            .defaultValue(18).min(1).sliderRange(0, 100).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<Integer> zigzagLegLength = this.sgGeneral.add(
        new meteordevelopment.meteorclient.settings.IntSetting.Builder()
            .name("zigzag-leg-length").description("How far to travel in blocks before turning (one leg of the zigzag).")
            .defaultValue(5000).min(100).sliderRange(500, 20000)
            .visible(() -> this.chunkLoadMode.get() == AreaLoaderModes.ZigZag).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<Integer> zigzagRowGap = this.sgGeneral.add(
        new meteordevelopment.meteorclient.settings.IntSetting.Builder()
            .name("zigzag-row-gap").description("How far to shift sideways in blocks between legs.")
            .defaultValue(192).min(16).sliderRange(16, 512)
            .visible(() -> this.chunkLoadMode.get() == AreaLoaderModes.ZigZag).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<String> saveLocation = this.sgGeneral.add(
        new meteordevelopment.meteorclient.settings.StringSetting.Builder()
            .name("save-name").description("The name to use for the folder that saves data. Leave blank to use 'default'.")
            .defaultValue("default").build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<Boolean> disconnectOnCompletion = this.sgGeneral.add(
    ...
    public final meteordevelopment.meteorclient.settings.Setting<Double> boatFlySpeed = this.sgGeneral.add(
        new meteordevelopment.meteorclient.settings.DoubleSetting.Builder()
            .name("boat-fly-speed").description("Speed in blocks per second for ETA calculation.")
            .defaultValue(30.0).min(1.0).sliderRange(1.0, 100.0)
            .visible(() -> this.chunkLoadMode.get() == AreaLoaderModes.Rectangle).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<OverworldFlightMode> overworldFlightMode = this.sgFlight.add(
        new Builder<OverworldFlightMode>().name("overworld-flight-mode")
            ...
            .defaultValue(OverworldFlightMode.PITCH40).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<Boolean> pitch40AutoFirework = this.sgFlight.add(
        new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
            .name("pitch40-auto-firework")
            .description("Enable auto-firework in Pitch40 when using PITCH40 mode.")
            .defaultValue(true)
            .visible(() -> this.overworldFlightMode.get() == OverworldFlightMode.PITCH40).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<Integer> spiralCornerReachDistance = this.sgFlight.add(
        new meteordevelopment.meteorclient.settings.IntSetting.Builder()
            .name("corner-reach-distance")
            .description("How close to a spiral corner before turning. The corner also turns once you pass its coordinate along the leg, so this is mainly a fallback.")
            .defaultValue(20).min(1).sliderRange(1, 100)
            .visible(() -> this.chunkLoadMode.get() == AreaLoaderModes.Spiral).build()
    );

    public final meteordevelopment.meteorclient.settings.Setting<Integer> spiralMaxOffTrackDistance = this.sgFlight.add(
        new meteordevelopment.meteorclient.settings.IntSetting.Builder()
            .name("max-off-track-distance")
            .description("How far off the spiral leg before saving state and navigating back to recover.")
            .defaultValue(500).min(20).sliderRange(50, 2000)
            .visible(() -> this.chunkLoadMode.get() == AreaLoaderModes.Spiral).build()
    );
    ...
    public final meteordevelopment.meteorclient.settings.Setting<Boolean> showGoalWaypoint = this.sgDebug.add(
        new meteordevelopment.meteorclient.settings.BoolSetting.Builder()
    ...
    public WWidget getWidget(GuiTheme theme) {
        ...
        clear.action = () -> {
            ...
            if (wasActive) this.info("Module was disabled. Activate it again to start fresh.", new Object[0]);
        };
        ...
        clearAll.action = () -> {
            ...
            if (wasActive) this.info("Module was disabled. Activate it again to start fresh with any mode.", new Object[0]);
        };
    ...
    private void rebuildMode(AreaLoaderModes mode) {
        switch (mode) {
            case Rectangle: this.currentMode = new Rectangle(); break;
            case Spiral: this.currentMode = new Spiral(); break;
            case ZigZag: this.currentMode = new ZigZag(); break;
        }
    }

    private void onModeChanged(AreaLoaderModes mode) {
        boolean wasActive = this.isActive() && this.mc.player != null && this.mc.world != null;
        if (wasActive) this.currentMode.onDeactivate();
        this.rebuildMode(mode);
        if (wasActive) this.currentMode.onActivate();
    }
